...
export default function ArticleGrid() {
  return (
    <Box sx={{ maxWidth: 1200, mx: "auto", px: { xs: 2, md: 0 }, py: { xs: 6, md: 9 } }}>
      <Typography component="h2" sx={{ color: "#3d3d3d", fontSize: 28, fontWeight: 700 }}>Latest articles</Typography>
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(3, 1fr)" }, gap: 3, mt: 3 }}>
        {articles.map(([title, text], index) => (
          <Box key={title} sx={{ p: 3, minHeight: 245, backgroundColor: index === 1 ? "#f2f9f3" : "#fafafa" }}>
